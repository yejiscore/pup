...
const AddDog = ({ handleAddDog }: { handleAddDog: () => void }) => {
  ...
  const [isNeutered, setIsNeutered] = useState('yes');
  const [dogImage, setDogImage] = useState(dogWhiteIcon);
  ...
  const { mutate: createDog } = useMutate('/dog', '/dog', 'post');

  const handleSave = async () => {
    const formattedBirthDate = formatBirthDate(dogBirth);
    let imageUrl = '';
    if (uploadData.walkingPhotos[0]) {
  ...
  return (
    <FormContainer>
      ...
      <ButtonWrapper>
        <AddButton onClick={handleAddDog}>취소</AddButton>
        <CancleButton onClick={handleSave}>저장하기</CancleButton>
      </ButtonWrapper>
    </FormContainer>
  );
};
